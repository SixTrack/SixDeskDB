...
<<<<<<< HEAD
  ...
  for angle in np.unique(tmp['angle']):
    for seed in np.unique(tmp['seed']):
      ...
      f.write('%s %s %s %s %s %s %s %s %s \n'%(studyName,seed,angle,achaos,achaos1,alost1,alost2,rad*inp['sigx1'][0],rad*inp['sigx1'][iel]))
  f.close()
  ...
  for angle in np.unique(final['angle']):
    ...
    f1.write('%s %.2f %.2f %.2f %.2f %.0f %.2f %.2f \n'%(studyName,angle, mini , mean, maxi,nega ,  Amin, Amax))
...
if __name__ == "__main__":
  ...
  for o, a in opts:
    if o in ("-h", "--help"):
      print "use: DA_FullStat_v2 <study_name>"
      sys.exit(0)
  if len(args)<1 :
