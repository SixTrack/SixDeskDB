...
class SixDeskDir(object):
  ...
  def get_num_fort10(self):
    return len(list(self.gen_job_params()))
  ...
  def iter_job_results(self,seed,tunex,tuney,amp1,amp2,turns,angle):
    params=seed,tunex,tuney,amp1,amp2,turns,angle
    dn=self.get_job_dirname(*params)
    fn=os.path.join(dn,'fort.10.gz')
  ...
  def iter_job_dirnames(self):
    for ppp in self.gen_job_params():
