    ...
    for seed in range(1, nSeeds+1):
        for angle in range(1, nAngles+1): 
            ...
            if not os.path.exists(path):
                os.makedirs(path)
            # plot_averem(path,seed,angle,nturns,a0,a1)
            # plot_kvar(path,seed,nturns,a0,a1,exp)
            # plot_distance(path,seed,angle,nturns,a0,a1)
            # plot_maxslope(path,seed,angle,nturns,a0,a1)
            # plot_smear(path,seed,angle,nturns,a0,a1)
            # plot_survival(path,seed,angle,nturns,a0,a1)
...
if __name__ == '__main__':
    ...
    exp = 5
    expDir="e%d" %exp
    createStructuresAndPlots(nSeeds, nAngles, a0, a1, nturns,exp)
