...
def PlotCompDaVsTurns(db,dbcomp,ldat,ldaterr,lblname,complblname,ampmaxsurv,ampmindavst,ampmaxdavst,tmax,plotlog,plotfit,fitndrop):
  ...
  #remove all files
  if(plotlog):
    files=('DA_comp_log.png DAsurv_comp.png DA_comp_log.%s.png DAsurv_comp.%s.png'%(turnse,turnse)).split()
  else:
    files=('DA_comp.png DAsurv_comp.png DA_comp.%s.png DAsurv_comp.%s.png'%(turnse,turnse)).split()
  clean_dir_da_vst(db,files)# create directory structure and delete old files if force=true
# start analysis
  if(not db.check_seeds()):
    print('Seeds are missing in database!')
  for seed in db.get_db_seeds():
    ...
    for tune in db.get_tunes():
      if(seed in dbcomp.get_db_seeds() and tune in db.get_tunes()):
        ...
        pl.savefig('%s/DAsurv_comp.%s.png'%(dirname,turnse))
        print('... saving plot %s/DAsurv_comp.%s.png'%(dirname,turnse))
        ...
        if(plotlog==True):
          pl.savefig('%s/DA_comp_log.%s.png'%(dirname,turnse))
          print('... saving plot %s/DA_comp_log.%s.png'%(dirname,turnse))
        else:
          pl.savefig('%s/DA_comp.%s.png'%(dirname,turnse))
          print('... saving plot %s/DA_comp.%s.png'%(dirname,turnse))
